 ...
 */
public class NovoAdministrador implements ActionListener {

    private Janela janela;

    public NovoAdministrador(Janela janela) {
        this.janela = janela;
    }
        ...
        try {
            ...
            if (admList != null) {
                ...
                for (Administrador el : admList) {
                    if (el.getNome().equals(janela.getNomeCadastro().getText())) {
                        JOptionPane.showMessageDialog(janela, "Nome já existente!", "Erro", 2);
                        return;
                    }
                }
            }

            Administrador adm = new Administrador(janela.getNomeCadastro().getText().trim(), janela.getSenhaCadastro().getText(), 0);

            admList.add(adm);

            ArquivoController.escreverArquivo(JSON.toJson(admList));

            JOptionPane.showMessageDialog(janela, "Cadastro realizado com sucesso!", "Concluído", 1);

        } catch (Exception ex) {
            System.err.println(ex);
            JOptionPane.showMessageDialog(janela, "Não foi possível concluir o cadastro");
        }
    }
